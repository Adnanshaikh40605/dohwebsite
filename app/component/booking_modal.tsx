...
const BookingModal = ({ show, onClose, bookingType }: any) => {
  const pathname = usePathname();
  
  const getRedirectUrl = (city: string) => {
    const baseUrl = (() => {
      switch (bookingType) {
        case "local":
          return city === "mumbai" ? "/booking-mumbai/" : "/booking-pune/";
        case "outstation":
          return city === "mumbai" ? "/outstation-booking/?city=mumbai" : "/outstation-booking/?city=pune";
        case "outstation-drop":
          return city === "mumbai" ? "/outstation-drop-booking/?city=mumbai" : "/outstation-drop-booking/?city=pune";
        default:
          return city === "mumbai" ? "/booking-mumbai/" : "/booking-pune/";
      }
    })();
    
    // Add referrer parameter to the URL
    const separator = baseUrl.includes('?') ? '&' : '?';
    return `${baseUrl}${separator}referrer=${encodeURIComponent(pathname)}`;
  };
  ...
  const getModalTitle = () => {
    switch (bookingType) {
      ...
      case "outstation":
        return "Select Your City for Outstation";
      case "outstation-drop":
        return "Select Your City for Outstation Drop";
      default:
        return "Select Your City";
    }
  };
  ...
  return (
    <div>
      ...
      <Modal
        show={show}
        onHide={onClose}
        centered={false} // Disable Bootstrap's default centering
        className="mobile-centered-modal"
      >
        <Modal.Body className="booking-modal-body">
          <div className="modal-border custom-border p-3">
            <div className="d-flex justify-content-between">
              <h5 className="booking-modal-heading">{getModalTitle()}</h5>
              <svg
            ...
            <div className="row justify-content-center text-center align-items-center">
              <div className="col-6 city-option">
                <Link
                  href={getRedirectUrl("mumbai")}
                  className="booking-modal-icon text-decoration-none"
                  id="mumbaiBooking"
                  onClick={onClose}
                >
                  <div className="booking-modal-icon-wrapper">
                    <img
                      src="/mumbai-icon1.svg"
                      alt="Mumbai city"
                      className="city-icon"
                    />
                  </div>
                  <p className="booking-modal-city-name">Mumbai</p>
                </Link>
              </div>
              <div className="col-6 city-option">
                <Link
                  href={getRedirectUrl("pune")}
                  className="booking-modal-icon text-decoration-none"
                  id="puneBooking"
                  onClick={onClose}
                >
                  ...
                  <p className="booking-modal-city-name">Pune</p>
                </Link>
              </div>
            </div>
          </div>
