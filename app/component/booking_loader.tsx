...
export default function BookingLoader({ id = "" }) {
  const [showAlertAndButton, setShowAlertAndButton] = useState(false);

  useEffect(() => {
    // After 3 seconds, show the alert and the button together
    const timer = setTimeout(() => {
      setShowAlertAndButton(true);
    }, 3000);
    ...
    <section id={`${id}`}>
      <div className="searching-div">
        <div className="loading loading04">
          ...
          <span>N</span>
          <span>G</span>
          <h5 className="mt-3 font-italic fs-6">The Drivers</h5>
        </div>
        {/* Conditional rendering with animation */}
        {showAlertAndButton && (
          <div
            className="alert-and-button fadeIn"
            style={{
              animation: "fadeIn 1s ease-out",
            }}
          >
            <div
              className="alert alert-success fs-6 fw-normal mt-4 m-2"
              style={{
                border: "1px solid",
                borderRadius: "30px",
              }}
            >
              <p className="mb-0 d-flex justify-content-center gap-2">
                <span className="me-2">
                  <svg
                    height="32px"
                    width="32px"
                    ...
                    <g id="Graphics-_x2F_-App-Icons-_x2F_-WhatsApp">
                      <g id="Icon_6_">
                        ...
                        >
                          ...
                          <stop offset=".686" stopColor="#22CC40" />
                ...
                Please check your WhatsApp for the booking confirmation details.{" "}
                <br />( Note: It may take up to 2 minutes to confirm the
                booking.
                <br />
            ...
            <div className="text-end mt-5 me-3">
              <Link
                className="btn btn-warning"
                style={{ border: "1px solid" }}
                href="/booking-details/"
              >
                Click here to go my bookings
              </Link>
            </div>
          </div>
        )}
