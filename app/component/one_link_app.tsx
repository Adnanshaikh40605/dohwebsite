...
const AppNotify = () => {
  ...
  useEffect(() => {
    // Check localStorage to see if the user has already dismissed the toast
    const isMobile = window.innerWidth <= 768;
    const toastDismissed = sessionStorage.getItem("toastDismissed");

    // Only show the toast if it hasn't been dismissed yet and the user is on mobile
    if (isMobile && !toastDismissed) {
  ...
  const handleCloseToast = () => {
    // Set the toast as dismissed in localStorage so it won't show again
    sessionStorage.setItem("toastDismissed", "true");
    setShowToast(false);
  };

  if (!showToast) {
    return null; // Don't render the toast if it's closed or not on mobile
  }

  return (
    <div className="py-3 d-flex justify-content-center">
      <div className="toast-app d-flex d-md-flex d-lg-none d-sm-flex">
        ...
        <div className="app-icon">
          <Image src="/only-logo.svg" alt="App Icon" width={25} height={25} />
        </div>
        <div className="promo-text">
          <h6>Get the app</h6>
          <p className="mb-0">Enjoy seamless driver services with our app!</p>
        </div>
        <div className="button-group">
          <Link
            href="https://driveronhire.onelink.me/Uy5P/gejeikwj"
            className="useAppButton"
          >
            Use app
          </Link>
        </div>
